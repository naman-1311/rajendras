    ...
    <section className="bg-[#0b0c0c] py-24 lg:py-28" ref={ref}>
      <div className="w-full pl-6 pr-6 sm:pl-10 lg:pl-[100px] lg:pr-12">
        ...
        >
          ...
          <h2 className="text-[28px] sm:text-[34px] lg:text-[46px] font-medium text-[#e9e6df] font-heading leading-[1.08]">
            The Principles That Drive Us
          </h2>
        </motion.div>

        {/* Cards grid */}
        <div className="grid grid-cols-2 gap-y-8 sm:grid-cols-3 lg:grid-cols-5 lg:flex lg:divide-x divide-white/10 w-full">
          {VALUES.map(({ icon: Icon, title, desc, hideOnMobile }, i) => (
            <motion.div
              key={title}
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: i * 0.1 }}
              className={`flex flex-1 flex-col items-center text-center px-4 py-4 lg:py-0 w-full ${hideOnMobile ? 'hidden lg:flex' : ''}`}
            >
